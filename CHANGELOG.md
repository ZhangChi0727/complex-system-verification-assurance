...
### ARINC v4.3 third-handshake candidate

- Recorded PR #14 ordinary merge commit `48dd8232b7efe6b0dba3fcb75dfc154d034d2b0b` as the first Candidate GVS Core 0.3 method-definition identity; retained `196cfc…` only as authoring provenance.
- Ingested the immutable ARINC v4.3 migration release: baseline ID `RB-2026-001-v4.3`, ordinary merge `523d42bf03a1135b3d63a00bfb47d3b879d3927e`, annotated release tag `v4.3` and tag object `28312fd…`; historical v4.2.1 remains frozen pre-framework provenance.
- Added the migration evidence return with eight commit-bound SHA-256 records, natural-person Review 5029797924 recorded truthfully as platform `COMMENTED` / body `APPROVE`, final-head and merge CI, 18/18 + 7 reconciliation, explicit absence of Project Configuration/execution manifest and returned findings.
- Defined the compatibility activation rule: pre-activation `NOT-DETERMINED`; activation by independent approval of the unchanged PR #15 head plus ordinary two-parent merge; post-activation `REVIEWED-COMPATIBLE-WITH-QUALIFICATION` under Q-01–Q-09, with the merge SHA as the immutable method-disposition identity. `NOT-EXERCISED`, RQ8 `Open` and architecture `OPEN-CANDIDATE` are unchanged.
- Hardened repository integrity CI to reject duplicate R/A rows, path/bytes/SHA/commit-locator misassociation, swapped hashes, incorrect tag object/peeled target/review identity and mutable locators; the checker and documents now share the same PR #15 activation semantics.
