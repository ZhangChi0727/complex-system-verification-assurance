...
- Proposed `REVIEWED-COMPATIBLE-WITH-QUALIFICATION` under Q-01–Q-09 for independent review. Until that exact PR head is approved and merged, formal compatibility remains `NOT-DETERMINED`; `NOT-EXERCISED`, RQ8 `Open` and architecture `OPEN-CANDIDATE` are unchanged.
- Extended repository integrity CI and negative tests to reject identity/tag mixing, row omission or strengthening, unqualified compatibility, premature Configuration/evaluation promotion, protected research deltas and prohibited/private artefacts.
