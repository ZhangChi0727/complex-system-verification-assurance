...
- Added a research innovation statement: four-layer innovation claims (thesis / architecture / mechanism / implementation) each anchored to gaps, annex objects or RQs with validation vehicles; explicit work boundary separating what standards provide (study / abstraction / summarization only) from what this research creates; explicit non-claims; and a methodology–instance decoupling contract (methodology detailed to implementation-framework level, instances executed in external repositories referencing this repo's stable object IDs).
- Added a HANDOFF workspace (current progress snapshot + next plan) maintained per merged PR; not a source of truth.
- Froze the instance-independent (generic-layer) standards research list: synced IEEE 1012, ISO/IEC/IEEE 24641, 24748-6, 15939 and 16326 from the ISO 24748-1 note follow-up register into the baseline; added ISO/IEC/IEEE 15026 series (Parts 1/3/4) and 29119 core parts (1/3/4) as companions; additions now require five-source §25 gap-priority rescoring with recorded rationale. Domain-profile, instance-standard and execution-technology sources remain demand-gated and do not violate the freeze.
