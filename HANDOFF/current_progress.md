...
- **Integration status:** Draft PR #9 is open; independent normative-review corrections are applied in the rewritten four-commit integration branch; external re-review is the current stop point
...
ISO-G07 information-item schema、ISO-G04 Oracle、ISO-G02B coverage、ISO-G03B sufficiency、ISO-G05 selection、ISO-G06 closure 与 ISO-G08 model evidence 继续保持可见。Rewritten Commit 3/4 已同步到 Draft PR #9，PR-level four-commit integrity and repository checks pass；当前停点是 external re-review。只有复审确认后才能转 Ready 并使用 ordinary merge commit。Merge SHA 与最终 `main` 状态只能在合并后记录，且不会为自引用另建非计划提交。
