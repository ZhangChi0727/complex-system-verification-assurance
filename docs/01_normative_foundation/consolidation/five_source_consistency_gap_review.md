...
本 annex 是 §22–§23 晋级决定及后续 research-delta candidate 的对象级 provenance 权威登记，不改变已经完成评审的冻结结论。目的：使 `PROMOTE ≠ source-native` 成为可查询事实，防止 framework-defined 对象在后续 Phase 4/5/8 中被误当作标准原生信息项。列含义：**Source-native** = 是否为已研究来源的原生术语/对象；**Non-aviation basis** = 是否存在非航空来源依据（generic 侧审计列）；**Schema gate** = 进入 executable schema 前必须满足的条件。`POST-v0.2 CANDIDATE` 只表示本轮研究建议，须经独立评审后才能成为冻结基线。
...
| Requirement Set | POST-v0.2 CANDIDATE — GENERIC CORE aggregate/view | Yes（ISO 29148, 5.2.6） | Direct | separate set-quality assessment | executable representation/cardinality | Independent review + ISO 15289 |
| Verification Criterion | POST-v0.2 CANDIDATE — GENERIC CORE concept | Yes（ISO 29148, 6.5.2.2） | Direct | relation-bearing concept between obligation/action/result | entity vs value/relation；cardinality | Independent review + ISO-G07B/REQ-G01 |
...
| Evidence Item | POST-v0.2 CANDIDATE — GENERIC CORE | Yes（ISO 15026-2, 5.3.2） | Direct | artefact + applicability + uncertainty + assumptions | identity/cardinality/admission workflow | Independent review + ISO 15289/15026-1 |
| Claim / Argument | GENERIC CORE; POST-v0.2 refinement candidate | Claim dependency-open；Argument direct（ISO 15026-2, 5.3.3/5.3.5） | Direct/Dependency open | recursive support, inference/evidence-reference alternatives | full Claim ontology；argument-quality assessment | Independent review + ISO 15026-1 / ASC-G03 |
| Supported Claim / Inference / Context / Undeveloped Argument | POST-v0.2 CANDIDATE — GENERIC CORE structure | Yes（ISO 15026-2, 3.1.4/3.1.7/5.3.1/5.3.4–5.3.5） | Direct | recursive assurance-case abstract structure | identity/reuse/cardinality/validity rules | Independent review + ISO 15026-1 |
| Narrative Introduction / Assurance Case Report | POST-v0.2 CANDIDATE — information-item/view | Yes（ISO 15026-2, 3.1.2/5.2/5.3.6） | Direct | report/index view distinct from whole Assurance Case | snapshot/version/assembly rules | Independent review + ISO 15289 / ASC-G04 |
...
|---|---|---|
| ISO/IEC/IEEE 29148:2018 | Requirements engineering, requirement quality, verification interface and requirement-information content | Strengthens Requirement/Requirement Set and requirement-to-method/criteria traceability; adds conceptual BRS/StRS/SyRS/SRS views |
| ISO/IEC/IEEE 15026-2:2022 | Assurance-case abstract structure | Refines Evidence Item and promotes Supported Claim, Inference, Context, Undeveloped Argument and Narrative Introduction |
...
Need / Goal / Objective
  ...
  → Evidence Item
  → leaf Argument or recursive Inference
  → Supported Claim
...
The arrows spanning Requirement/Basis to Verification Obligation and Result/Artefact to Evidence Item remain framework-defined bridges. A Requirement is not a Claim; a Result is not automatically Evidence; a trace link is not an Argument; structural completeness is not Claim truth, sufficiency or acceptance.
...
| Evidence Item | Refined to artefact + applicability + uncertainty + assumptions |
...
ISO-G07 is split: conceptual information-item/view taxonomy is resolved (`ISO-G07A`), while executable schema/cardinality/interoperability remains open (`ISO-G07B`). New high-priority dependencies are ISO/IEC/IEEE 15289 and ISO/IEC/IEEE 15026-1. The latter is normatively referenced by 15026-2 for assurance/claim/uncertainty vocabulary. ISO 29148's process clauses cite ISO 15288:2015, so exact mapping to the repository's ISO 15288:2023 baseline is also open.
...
**POST-v0.2 RESEARCH DELTA READY FOR INDEPENDENT REVIEW.** The conceptual architecture is coherent and the required dependencies/non-claims are explicit. No executable schema, universal evidence-sufficiency threshold, assurance acceptance or certification conclusion is claimed.
