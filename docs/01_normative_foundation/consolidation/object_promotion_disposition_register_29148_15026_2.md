...
Promotion requires a stable semantic identity, an explicit source locator, compatibility with the five-source ontology, and a clear generic/profile boundary. Promotion here is conceptual only; it does not freeze database fields, cardinalities, state machines or tooling.

Section 2 records the evidence summary. Section 3 records this research round's review-candidate dispositions and uses only the task-controlled vocabulary: `KEEP AS GENERIC CORE`, `PROMOTE TO GENERIC CORE`, `KEEP AS EXTENSION POINT`, `DEMOTE TO PROFILE/PROJECT`, `SPLIT`, `MERGE`, `RENAME`, `REFERENCE ONLY`, `KEEP OPEN`, `REMOVE`. The authoritative cross-round provenance register remains §28 of `five_source_consistency_gap_review.md`; its `POST-v0.2 CANDIDATE` entries are not frozen until independent review closes.
...
| OP-23 | Evidence admission | Generic assessment/relation | PROMOTE CONCEPTUALLY | 15026-2, 5.3.2/5.3.5 | Does not imply sufficiency or credit |
...
| OP-26 | 15289-compatible executable schema | Information architecture | DEPENDENCY OPEN | No local 15289 source/study | Do not infer from current Markdown/YAML |
...
| Verification Result | Generic Core | Result/objective information remains upstream of evidence admission | KEEP AS GENERIC CORE | Cross-source; 29148, 6.5.2.2–6.5.2.3 | Result ≠ Evidence Item ≠ Claim | V7/V8/evidence; schema OPEN; review required |
...
| Evidence Item | Not fully distinguished | Four-field source-native semantics found | PROMOTE TO GENERIC CORE | Source-native; 15026-2, 5.3.2 | Must represent artefact/applicability/uncertainty/assumptions; existence ≠ claim support | Info/evidence model; schema BLOCKED by 15289/15026-1; review required |
