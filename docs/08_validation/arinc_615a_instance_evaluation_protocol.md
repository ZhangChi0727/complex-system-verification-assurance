...
13. How do information population, trace/mapping review effort, execution/storage/indexing cost and layer contamination behave as the bounded instance scale increases?
...
| scalability | How does the bounded instance behave as verification information and governance populations grow? | counted CRS/obligation/case/procedure/observation/evidence populations; trace/mapping query logs; execution, storage, indexing and manual-review effort | size-stratified runs/reviews with population counts, elapsed effort, storage/index measures, omissions and unresolved-link counts | growth causes unbounded review effort, missed obligations/links, unauditable populations or leakage across Core/Profile/Binding/Configuration | measured scalability characteristics and limits for the tested ranges only | universal scalability or software performance alone establishes framework scalability | scale/cost/omission/layer-contamination finding with tested bounds | cross-instance evaluation owner plus method reviewer |
...
| interface conformance | Does a Binding satisfy semantic inputs/outputs/invariants/failure rules independently of implementation technology? | semantic contract, binding specification and conformance record | contract checklist and negative/failure-path tests | implementation-specific API silently replaces semantic obligation | specified-binding contract checks satisfied/qualified/not satisfied | overall compatibility verdict or API/schema is the Core definition | binding defect or qualification; compatibility remains `NOT-DETERMINED` until the third handshake and independent review | compatibility reviewer |
